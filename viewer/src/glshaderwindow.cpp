...
glShaderWindow::glShaderWindow(QWindow *parent)
    ...
    : OpenGLWindow(parent), modelMesh(0),
      ...
      gpgpu_vertices(0), gpgpu_normals(0), gpgpu_texcoords(0), gpgpu_colors(0),
	  gpgpu_indices(0),
...
void glShaderWindow::openSceneFromFile()
{
	...
	QFileDialog dialog(
		0, "Open Scene", workingDirectory, "*.off *.obj *.ply *.3ds"
	);
...
void glShaderWindow::openNewTexture()
{
	...
	QFileDialog dialog(
		0, "Open texture image", 
		workingDirectory + "../textures/", "*.png *.PNG *.jpg *.JPG *.tif *.TIF"
	);
	dialog.setAcceptMode(QFileDialog::AcceptOpen);
	QString filename;
...
void glShaderWindow::openNewEnvMap()
{
	...
	QFileDialog dialog(
		0, "Open environment map image", 
		workingDirectory + "../textures/", "*.png *.PNG *.jpg *.JPG"
	);
	dialog.setAcceptMode(QFileDialog::AcceptOpen);
	QString filename;
	...
	if (ret == QDialog::Accepted) {
		...
		if (environmentMap) {
			...
			environmentMap->setMinificationFilter(
				QOpenGLTexture::LinearMipMapLinear
			);
...
void glShaderWindow::updateShininess(int shininessSliderValue)
{
	...
	if (blinnPhong)
		...
		shininess = (float) shininessSliderValue / 200.0;
...
/* -------------------------------------------------------------------------- */
	...
	connect(
		lightSlider, SIGNAL(valueChanged(int)), 
		this, SLOT(updateLightIntensity(int))
	);
	...
	connect(
		lightSlider, SIGNAL(valueChanged(int)), 
		lightLabelValue, SLOT(setNum(int))
	);
...
/* -------------------------------------------------------------------------- */
	...
	connect(
		shininessSlider, SIGNAL(valueChanged(int)), 
		this, SLOT(updateShininess(int))
	);
	...
	connect(
		shininessSlider, SIGNAL(valueChanged(int)),
		shininessLabelValue, SLOT(setNum(int))
	);
...
/* -------------------------------------------------------------------------- */
	...
	connect(
		etaSlider, SIGNAL(valueChanged(int)), 
		etaLabelValue, SLOT(setNum(int))
	);
...
void glShaderWindow::createSSBO() 
{
	...
	glBufferData(
		GL_SHADER_STORAGE_BUFFER, 
		modelMesh->normals.size() * sizeof(trimesh::vec), 
		&(modelMesh->normals.front()),
		GL_STATIC_READ
	);
	...
	glBufferData(
		GL_SHADER_STORAGE_BUFFER, 
		modelMesh->colors.size() * sizeof(trimesh::Color), 
		&(modelMesh->colors.front()), 
		GL_STATIC_READ
	);
...
void glShaderWindow::bindSceneToProgram()
{
	...
	// If we're doing GPGPU, the VAO is very simple: 4 vertices for a large
	// square covering the screen.
	if (isGPGPU) {
		m_numFaces = 2;
		...
		m_numFaces = modelMesh->faces.size();
	...
	if (!isGPGPU)
		...
		m_vertexBuffer.allocate(gpgpu_vertices, 4 * sizeof(trimesh::point));

	m_indexBuffer.setUsagePattern(QOpenGLBuffer::StaticDraw);
	m_indexBuffer.bind();
	if (!isGPGPU) 
		m_indexBuffer.allocate(
			&(modelMesh->faces.front()), m_numFaces * 3 * sizeof(int)
		);
	else
		m_indexBuffer.allocate(gpgpu_indices, m_numFaces * 3 * sizeof(int));
	...
	if (modelMesh->colors.size() > 0) {
		...
		if (!isGPGPU)
			...
			m_colorBuffer.allocate(gpgpu_colors, 4 * sizeof(trimesh::Color));
	}
	...
	if (!isGPGPU)
		...
		m_normalBuffer.allocate(gpgpu_normals, 4 * sizeof(trimesh::vec));
	...
	if ((modelMesh->texcoords.size() > 0) || isGPGPU) {
		...
		if (!isGPGPU)
			m_texcoordBuffer.allocate(
				&(modelMesh->texcoords.front()), 
				modelMesh->texcoords.size() * sizeof(trimesh::vec2)
			);
		else
			m_texcoordBuffer.allocate(
				gpgpu_texcoords, 4 * sizeof(trimesh::vec2)
			);
	}
	...
	if (modelMesh->texcoords.size() > 0) {
		m_texcoordBuffer.bind();
		shadowMapGenerationProgram->setAttributeBuffer(
			"texcoords", GL_FLOAT, 0, 2
		);
	...
	// Bind ground VAO to ground program as well. We create a VAO for the
	// ground from scratch.
	ground_vao.bind();
	...
	for (int i = 0; i < numR; i++) {
		for (int j = 0; j < numTh; j++) {
			...
			g_vertices[p] = center + trimesh::point(
				rad * cos(theta), -groundDistance * radius, rad * sin(theta), 0
			);
	...
	ground_vertexBuffer.setUsagePattern(QOpenGLBuffer::StaticDraw);
	ground_vertexBuffer.bind();
	ground_vertexBuffer.allocate(
		g_vertices, g_numPoints * sizeof(trimesh::point)
	);
	...
	ground_texcoordBuffer.allocate(
		g_texcoords, g_numPoints * sizeof(trimesh::vec2)
	);
...
void glShaderWindow::initializeTransformForScene()
{
	...
	m_perspective.perspective(
		45, (float) width() / height(), 0.1 * radius, 20 * radius
	);
...
void glShaderWindow::openScene()
{
	...
	if (!modelMesh) {
		QMessageBox::warning(
			0, tr("qViewer"), tr("Could not load file ") + modelName,
			QMessageBox::Ok
		);
		openSceneFromFile();
	}
...
void glShaderWindow::saveScene()
{
	...
	QFileDialog dialog(
		0, "Save current scene", workingDirectory,
	...
	if (!filename.isNull()) {
		if (!modelMesh->write(qPrintable(filename))) {
			QMessageBox::warning(
				0, tr("qViewer"), tr("Could not save file: ") + filename,
				QMessageBox::Ok
			);
		}
	}
...
void glShaderWindow::saveScreenshot()
{
	...
	QFileDialog dialog(
		0, "Save current picture", workingDirectory, "*.png *.jpg"
	);
	dialog.setAcceptMode(QFileDialog::AcceptSave);
	...
	if (ret == QDialog::Accepted) {
		...
		if (!pixmap.save(filename)) {
			QMessageBox::warning(
				0, 
				tr("qViewer"), tr("Could not save picture file: ") + filename, 
				QMessageBox::Ok
			);
		}
	}
...
void glShaderWindow::setWindowSize(const QString& size)
{
	...
	parent()->resize(
		parent()->width() - width() + dims[0].toInt(), 
		parent()->height() - height() + dims[1].toInt()
	);
...
void glShaderWindow::loadTexturesForShaders()
{
	...
	if (
		...
		(hasComputeShaders && 
		 compute_program->uniformLocation("colorTexture") != -1)
		) 
	{
		glActiveTexture(GL_TEXTURE0);
		// The shader wants a texture. We load one.
	...
	if (
		(m_program->uniformLocation("envMap") != -1) || 
		(hasComputeShaders && compute_program->uniformLocation("envMap") != -1))
	{
		...
		if (m_program->uniformLocation("permTexture") != -1) {
			permTexture = new QOpenGLTexture(
				QImage(pixels, 256, 256, QImage::Format_RGBA8888)
			);
			if (permTexture) {
	...
	if (hasComputeShaders) {
		// We bind the texture generated, to texture unit 2 (0 is for the
		// texture, 1 for the env map) 2 is also used by the shadow map.
		// If you need both, move one to texture unit 3.
		...
		glTexImage2D(
			...
			shadowMapDimension, shadowMapDimension, 
			0, GL_DEPTH_COMPONENT, GL_FLOAT, 0
		);
		
		// Create a framebuffer object, you need to delete them when the
		// program exits.
		if (shadowMap_fboId == 0)
			glGenFramebuffers(1, &shadowMap_fboId);
		glBindFramebuffer(GL_FRAMEBUFFER, shadowMap_fboId);

		/*
			Create a renderbuffer object to store depth info.
			NOTE: A depth renderable image should be attached the FBO for depth
			test. If we don't attach a depth renderable image to the FBO, then
			the rendering output will be corrupted because of missing depth
		...
		*/
		if (shadowMap_rboId == 0)
			glGenRenderbuffers(1, &shadowMap_rboId);
		...
		glFramebufferTexture2D(
			...
			GL_TEXTURE_2D, shadowMap_textureId, 0
		);

		// Attach a renderbuffer to depth attachment point.
		//glFramebufferRenderbuffer(
		//	GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER_EXT, rboId);
...
void glShaderWindow::initialize()
{
	...
	qDebug(
		"OpenGL initialized: version: %s GLSL: %s", 
	...
	m_program = prepareShaderProgram(
		shaderPath + "gpgpu_fullrt.vert", shaderPath + "gpgpu_fullrt.frag"
	);
	compute_program = prepareComputeProgram(shaderPath + "gpgpu_fullrt.comp");

	shadowMapGenerationProgram = prepareShaderProgram(
		shaderPath + "h_shadowMapGeneration.vert", 
		shaderPath + "h_shadowMapGeneration.frag"
	);
...
void glShaderWindow::resize(int x, int y)
{
	...
	// For GPGPU: since computeResult is the size of the window, it must be
	// recreated at each window resize.
	if (computeResult) {
		computeResult->release();
		computeResult->destroy();
		delete computeResult;
	...
	if (hasComputeShaders) {
		...
		m_program->bind();
#if 0
		computeResult = new QOpenGLTexture(
			QImage(pixels, x, y, QImage::Format_RGBA8888)
		);
		if (computeResult) {
			computeResult->setWrapMode(QOpenGLTexture::MirroredRepeat);
			computeResult->setMinificationFilter(QOpenGLTexture::Nearest);
	...
	// Normal case: we rework the perspective projection for the new viewport,
	...
	if (m_program && modelMesh) {
		...
		if (x > y)
			m_perspective.perspective(
				60, (float) x / y, 0.1 * radius, 20 * radius
			);
		else
			m_perspective.perspective(
				(240.0 / M_PI) * atan((float) y / x), (float) x / y, 
				0.1 * radius, 20 * radius
			);
...
QOpenGLShaderProgram *glShaderWindow::prepareShaderProgram(
...
)
{
	...
	if (!program)
		qWarning() << "Failed to allocate the shader";
...
QOpenGLShaderProgram *glShaderWindow::prepareComputeProgram(
...
)
{
	...
	bool result = program->addShaderFromSourceFile(
		QOpenGLShader::Compute, computeShaderPath
	);
	if (!result)
		qWarning() << program->log();
...
void glShaderWindow::setWorkingDirectory(
...
void glShaderWindow::mousePressEvent(QMouseEvent *e)
{
	...
	lastMousePosition = 
		(2.0 / m_screenSize) * 
		(QVector2D(e->localPos()) - QVector2D(0.5 * width(), 0.5 * height()));
...
void glShaderWindow::mouseMoveEvent(QMouseEvent *e)
{
	...
	switch (mouseButton) {
		case Qt::LeftButton: {
			...
			QVector4D ax = 
				m_matrix[matrixMoving].inverted() * QVector4D(1, 0, 0, 0);
			QVector4D ay = 
				m_matrix[matrixMoving].inverted() * QVector4D(0, 1, 0, 0);
			QQuaternion rx = 
				QQuaternion::fromAxisAndAngle(QVector3D(ax), dx * 100);
			QQuaternion ry = 
				QQuaternion::fromAxisAndAngle(QVector3D(ay), dy * 100);
		...
		case Qt::RightButton: {
			QVector2D diff = 
				0.2 * m_screenSize * (mousePosition - lastMousePosition);
			if (matrixMoving == 0) {
				QMatrix4x4 t;
				t.translate(
					diff.x() * modelMesh->bsphere.r / 100.0, 
					-diff.y() * modelMesh->bsphere.r / 100.0, 
					0.0
				);
				m_matrix[matrixMoving] = t * m_matrix[matrixMoving];
			} else if (matrixMoving == 1)
...
void glShaderWindow::timerEvent(QTimerEvent *e) { return; }
...
void glShaderWindow::render()
{
	...
	QVector3D lightPosition = 
		m_matrix[1] * 
		(m_center + 
			lightDistance * modelMesh->bsphere.r * QVector3D(0.5, 0.5, 1)
		);
	...
	if (hasComputeShaders) {
		// We bind the texture generated to texture unit 2
		// (0 is for the texture, 1 for the env map).
		glActiveTexture(GL_TEXTURE2);
		compute_program->bind();
		computeResult->bind(2);
		...
		compute_program->setUniformValue(
			"groundDistance", 
			groundDistance * modelMesh->bsphere.r - m_center[1]
		);
		...
		glDispatchCompute(
			worksize_x / compute_groupsize_x, 
			worksize_y / compute_groupsize_y, 
			1
		);
		...
		// Mainly because some models intersect with the ground.
		...
		shadowMapGenerationProgram->setUniformValue(
			"perspective", lightPerspective
		);
	...
	else if (m_program->uniformLocation("permTexture") != -1)
